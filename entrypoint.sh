...
echo -e "${INPUT_SSH_KEY}" >./gh-action/.github/workflows/_temp_ssh_key

# Change file access permissions.
chmod 600 ./gh-action/.github/workflows/_temp_ssh_key
...
scp -o StrictHostKeyChecking=no -v -i ./gh-action/.github/workflows/_temp_ssh_key -P "${INPUT_PORT}" -r ${INPUT_LOCAL_PATH} "${INPUT_USERNAME}"@"${INPUT_HOST}":"${INPUT_SERVER_PATH}"